...
// 手动输入标定好的相机内参
cv::Mat camera_matrix = (cv::Mat_<double>(3, 3) << 833.5051210437123, 0, 330.5683362465313,
                         0, 833.8074347617661,
                         255.7232077600928,
                         0, 0, 1);
// matrix_num2
// 相机畸变参数
cv::Mat dist_coeffs = (cv::Mat_<double>(1, 5) << 0.04509283803368063, 0.2234215792538128,
                       0.004863167176240346, 0.004636669832202732, 0); // matrix_num2
// Aruco 二维码的编码代号
...
void getMarker(cv::Mat &marker_image, vector<cv::Point2f> &marker_center, bool key)
{
    ...
    dictionary = cv::aruco::getPredefinedDictionary(cv::aruco::DICT_5X5_100); // 特别要注意
    if (!marker_image.empty())
    {
        ...
        cv::aruco::estimatePoseSingleMarkers(corners, 0.10, camera_matrix, dist_coeffs, rvecs,
                                             tvecs); // 0.086 marker 大小
        ...
        {
            cv::aruco::drawAxis(marker_image, camera_matrix, dist_coeffs, rvecs, tvecs,
                                0.1);
            // 画坐标轴
            Zc = tvecs[0][2] - 0.044;
            cout << "深度输出:" << Zc << endl;
